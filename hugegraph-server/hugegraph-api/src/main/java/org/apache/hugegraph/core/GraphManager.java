...
import org.apache.hugegraph.HugeAliasGraph;
...
public final class GraphManager {
    ...
    public GraphManager(HugeConfig conf, EventHub hub) {
        ...
        try {
            ...
            this.pdClient = DiscoveryClientImpl
                    .newBuilder()
                    .setCenterAddress(this.pdPeers) // pd grpc 端口
    ...
    private KvStore kvStoreInit() {
        // 创建或者加载系统图，此图对用户隐藏，只作为 KV 数据存储使用
    ...
    public Long getGraphStorage(String graphSpace, String graph) {
        // eg: DEFAULT/hugegraph/g
        // Storage usage units: kb
        // todo:zzz 要搞 pd 的部分先 mock 一手把
        //String graphSpaceName = String.join("/", graphSpace, graph, "g");
        //MetaDriver metaDriver = this.metaManager.metaDriver();
        //assert metaDriver instanceof PdMetaDriver;
        //PDClient pdClient = ((PdMetaDriver) metaDriver).pdClient();
        //try {
        //    return pdClient.getGraphStats(graphSpaceName).getStats()
        //                   .getApproximateSize();
        //} catch (PDException e) {
        //    LOG.error("Get graph '{}' storage information meet error {}",
        //              graphSpaceName, e);
        //}
        //return 0L;
        return 1111L;
    ...
    private void loadAliasGraph(Map<String, Map<String, Object>> graphConfigs) {
        // 加载图
        for (Map.Entry<String, Map<String, Object>> conf : graphConfigs.entrySet()) {
            ...
            try {
                this.createAliasGraph(parts[0], parts[1], config, false);
    ...
    private void loadGraphsFromMeta(
        ...
        // 加载别名图
        this.loadAliasGraph(aliasGraphConfigs);
    }
    ...
    // todo:zzz 这里和内部版区别在 id 和 role 但是这两个现在应该没啥用了，但是没有 pd 要用，需要保留吗
    private void serverStarted(HugeConfig config) {
        ...
        for (String graph : this.graphs()) {
            ...
            // todo：zzz 原本社区版是，我选择先不管，内部版打开看看
    ...
    private <T> void graphAddHandler(T response) {
        ...
        for (String graphName : names) {
            ...
            // 如果当前 server 注册的不是 DEFAULT 图空间，只接收已注册图空间下的图创建事件
            if (!"DEFAULT".equals(this.serviceGraphSpace) &&
    ...
    private void updateAliasGraph(HugeGraph hugeGraph, Map<String, Object> configs) {
        try {
            if (hugeGraph instanceof HugeAliasGraph) {
                HugeAliasGraph hugeAliasGraph = (HugeAliasGraph) hugeGraph;
                if (Objects.nonNull(configs) &&
                    configs.containsKey(CoreOptions.ALIAS_NAME.name())) {
                    String realGraphName =
                            configs.getOrDefault(CoreOptions.ALIAS_NAME.name(),
                                                 CoreOptions.ALIAS_NAME.defaultValue()).toString();
                    realGraphName = graphName(hugeAliasGraph.graphSpace(), realGraphName);
                    if (this.graphs.containsKey(realGraphName)) {
                        hugeAliasGraph.changeHugeGraph((HugeGraph) this.graphs.get(realGraphName));
                        LOG.info("Update alias graph success, aliasGraphName:{} realGraphName:{}",
                                 hugeAliasGraph.name(), realGraphName);
                    }
                }
                LOG.info("updateAliasGraph Update aliasGraph space:{} graph:{} configs:{}",
                         hugeAliasGraph.graphSpace(), hugeAliasGraph.name(), configs);
            }
        } catch (Exception e) {
            LOG.error("Failed to update alias graph, name:{}", hugeGraph.name(), e);
        }
    }
            ...
            for (String graphName : graphNames) {
                ...
                if (this.graphs.containsKey(graphName)) {
                    ...
                    if (graph instanceof HugeGraph) {
                        ...
                        // Whitebox.invoke() 将会自动开启事务，这里需要手动 commit
                        hugeGraph.tx().commit();
    ...
    public HugeGraph createAliasGraph(String graphSpace, String name,
        ...
        // 创建别名图 init 为 false 为 不需要初始化 store 相关类
    ...
    public void dropGraph(String graphSpace, String name, boolean clear) {
        ...
        if (clear) {
            ...
            // 非别名图的删除
            if (!(g instanceof HugeAliasGraph)) {
                /**
                 * close task scheduler before clear data,
                 * because taskinfo stored in backend in
                 * {@link com.baidu.hugegraph.task.DistributedTaskScheduler}
                 */
                try {
     ...
     * @param aliasId
     */
    public void updateAliasGraphAliasId(String graphSpace, String graphName,
                                        String aliasId) {
        try {
            Map<String, Object> configs =
                    this.metaManager.getGraphConfig(graphSpace, graphName);
            configs.put(CoreOptions.ALIAS_NAME.name(), aliasId);
            this.metaManager.updateGraphConfig(graphSpace, graphName, configs);
            this.metaManager.notifyGraphUpdate(graphSpace, graphName);
        } catch (Exception e) {
            LOG.warn("The graph not exist or local graph");
